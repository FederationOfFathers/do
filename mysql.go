...
func initQueries() {
	...
	findNeedXUID = mustPrepare(strings.Join([]string{
		...
		"FROM",
		"	members m",
		"	JOIN membermeta mm ON ( m.id = mm.member_ID AND mm.meta_key = 'xuid' )",
		"	JOIN membermeta mmm ON ( m.id = mmm.member_ID AND mmm.meta_key = '_xuid_last_check' )",
		"WHERE",
		"	xbl NOT IN('','**DISABLED**')",
		"	AND seen > ?",
	...
	findNeedGameFill = mustPrepare(strings.Join([]string{
		...
		"	AND seen > ?",
		"HAVING",
		"	mm.meta_value != '0' AND ( mmm.meta_value = '' OR mmm.meta_value < ? )",
		"ORDER BY",
		"	m.id ASC",
		"LIMIT 1",
	}, " "))
